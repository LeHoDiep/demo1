...
let totalQuestionsInRound = TOTAL_QUESTIONS;
...
function shuffleAnswersForQuestion(question) {
  ...
  return {
    ...question,
    answers: shuffledOptions.map((o) => o.answer),
...
// Start quiz
function startQuiz() {
  // Combine, remove duplicates, then randomize questions + answers for this round.
  const allQuestions = [...quizQuestions, ...additionalQuestions];
  const uniqueQuestions = deduplicateQuestions(allQuestions);
  const preparedQuestions = uniqueQuestions.map(shuffleAnswersForQuestion);

  totalQuestionsInRound = Math.min(TOTAL_QUESTIONS, preparedQuestions.length);
  currentQuestions = shuffleArray(preparedQuestions).slice(0, totalQuestionsInRound);
...
function showQuestion() {
  ...
  document.getElementById("progress-bar").style.width =
    `${((currentQuestion + 1) / totalQuestionsInRound) * 100}%`;
