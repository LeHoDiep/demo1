...
function renderList() {
  const articles = getArticles().sort((a, b) => a.order - b.order);
  listView.innerHTML = "";
...
function showDetail(id) {
  var art = getArticles().find(function (a) {
    return a.id === id;
  });
  if (!art) return;
...
function renderAdminList() {
  ...
  if (!container) return;
  var articles = getArticles().sort(function (a, b) {
...
function toggleCarousel(id, value) {
  var articles = getArticles().map(function (a) {
    if (a.id === id) {
      return Object.assign({}, a, { showInCarousel: value });
    }
    return a;
  });
  saveArticles(articles);
...
function deleteArticle(id) {
  var articles = getArticles().filter(function (a) {
    return a.id !== id;
  });
  // Re-order
  articles
...
// --- Init ---
// Check if URL has ?id= to show a specific article
(function () {
